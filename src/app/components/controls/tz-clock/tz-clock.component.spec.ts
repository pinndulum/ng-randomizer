...
describe('TzClockComponent', () => {
    ...
    it('uses daylight time for North American zones during DST', () => {
        ...
        expect(component.tzOffsetLabel).toBe('UTC-07:00');
    });
    ...
    it('uses standard time for North American zones outside DST', () => {
        ...
        expect(component.tzOffsetLabel).toBe('UTC-08:00');
    });

    it('describes the day part in the selected zone', () => {
        fixture.componentRef.setInput('zone', pacificZone);
        component.dt.set(new Date(Date.UTC(2026, 0, 1, 17)));
        fixture.detectChanges();

        expect(component.dayPart).toBe('Morning');
        expect(component.isNight).toBeFalse();
    });
});
